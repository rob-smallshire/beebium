...
// R1 P-to-H transfer tests using the real 65C02 CPU.
//
// The coprocessor writes bytes to the R1 P-to-H 24-byte FIFO via $FEF9.
// The host reads them from R1 data (offset 1).  The coprocessor polls R1
// status ($FEF8) bit 6 (space available) before each write.
//
// This exercises the coprocessor write path: CoprocessorCpu::tick() ->
// memory_.write() -> TubeUla::coprocessor_write().
...
// ============================================================================
// 6502 R1 polled write program (Coprocessor-to-Host)
// ============================================================================
//
// $0400: LDX #$00
...
// $040A: STA $FEF9        ; write R1 data
// $040D: INX
// $040E: CPX #NUM_BYTES
// $0410: BNE $0402        ; loop
// $0412: BRA *            ; halt

static void plant_r1_writer(CoprocessorMemoryMap& mem, uint8_t num_bytes) {
    plant(mem, CODE_ADDR, {
        0xA2, 0x00,                         // LDX #$00
        0x2C, 0xF8, 0xFE,                   // BIT $FEF8     (poll R1 status)
        0x50, 0xFB,                          // BVC $0402
        0xBD, 0x00, 0x05,                   // LDA $0500,X
        0x8D, 0xF9, 0xFE,                   // STA $FEF9     (write R1 data)
        0xE8,                                // INX
...
TEST_CASE("6502 R1 P2H: 200 bytes interleaved", "[tube][6502][r1]") {
    TubeUla tube;
    ...
    for (int i = 0; i < 10000000 && cpu.cpu().opcode_pc.w != 0x0412; ++i) {
        cpu.tick();
        if (host_read_idx < NUM_BYTES && (tube.host_read(0) & TubeUla::DATA_AVAILABLE)) {
            received[host_read_idx] = tube.host_read(1);
            ++host_read_idx;
        }
    }
...
TEST_CASE("6502 R1 P2H: 200 bytes, repeated 50 times", "[tube][6502][r1]") {
    constexpr uint8_t NUM_BYTES = 200;
