...
public:
    ...
    //
    // The CPU goes through M6502_Reset only. A 6502 RES preserves A, X, Y and the
    // N V D Z C flags, sets the interrupt-disable flag, lowers S by three (three
    // dummy stack reads) and loads PC from the reset vector at &FFFC. M6502_Init
    // is a power-on state (it memsets the CPU) and must not be used for a reset;
    // config/fns/interrupt_tfn were established by the power-on reset() and
    ...
    //
    // soft_reset() mutates CPU state (tfn, S, flags), so it MUST run while the
    // emulation loop is idle: otherwise a torn read of cpu.tfn, or a call through
    // a half-updated tfn, races against `(*cpu.tfn)(&cpu)` and can crash the
    // server (issues #27 / #39). with_emulation_paused() provides that exclusion.
    void break_up() {
        with_emulation_paused([this] {
            soft_reset();
        });
