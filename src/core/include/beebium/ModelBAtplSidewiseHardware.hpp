...
// BBC Model B fitted with the ATPL Sidewise sideways ROM/RAM expansion board.
//
// This emulates a specific historical product (ATPL Advanced Technology Products
// Ltd, 1983) rather than a notional board. The Sidewise expands the four stock
// sideways sockets to the full sixteen supported by the OS:
// - Slots 0-3 are the (relocated) motherboard sockets, ROM only.
// - Slots 4-14 are the board's ROM sockets (2764 / 27128 EPROMs or masked ROM).
// - Slot 15 is the board's single RAM/ROM socket. The real board splits it into
//   two 8K halves (15a/15b); we model it as one 16K slot that may be fitted with
//   RAM or a ROM at launch.
//
// Two behaviours distinguish this from a plain banked-memory board:
//
//  1. Slot-15 write-through. Any CPU write to the sideways region &8000-&BFFF is
//     latched into the slot-15 RAM directly, irrespective of and without
//     affecting the currently paged ROM. The RAM only reads back when slot 15 is
//     paged in via the &FE30 ROMSEL latch. Software such as the ROM Filing System
//     and ATPL's SPOOLIT printer buffer relies on this.
//
//  2. Write-protect. Slot 15 has a write-protect control (the S6 link, commonly
//     wired to a rear-panel switch) that inhibits writes to the RAM at runtime.
//
// Consistent with modelling a real board, slot population is fixed at launch (via
// CLI / preset, like fitting a chip) and cannot be reconfigured at runtime; the
// write-protect switch is the only runtime control. Battery backup is not
// modelled - it has no meaning in emulation.
//
class ModelBAtplSidewiseHardware {
public:
    // The single RAM/ROM socket. Slots 0-14 are ROM-only sockets.
    static constexpr uint8_t SLOT_15 = 15;

    // Machine identification and region names (compile-time constants)
    static constexpr std::string_view MACHINE_TYPE = "model-b-atpl-sidewise";
    constexpr std::string_view machine_type() const { return MACHINE_TYPE; }
    static constexpr std::string_view MACHINE_DISPLAY_NAME = "BBC Model B with ATPL Sidewise";
    static constexpr std::string_view MACHINE_DESCRIPTION = "Model B fitted with the ATPL Sidewise sideways ROM/RAM expansion board (16 slots; slot 15 RAM/ROM with write-through and write-protect)";
    ...
    // The Sidewise manual installs BASIC in socket 14 (the highest-priority 16K
    // socket), because slot 15 is reserved for the RAM/ROM socket.
    static constexpr uint8_t DEFAULT_LANGUAGE_SLOT = 14;
    // No DEFAULT_DFS_ROM: the board is empty expansion sockets with no filing
    // system of its own. A DFS (or any other FS ROM) is provisioned by a preset
    ...
    ModelBAtplSidewiseHardware(ViaPeripheral& system_peripheral, ViaPeripheral& user_peripheral)
    ...
    void write(uint16_t addr, uint8_t value) {
        // ATPL Sidewise slot-15 write-through: when slot 15 is fitted with RAM, a
        // write anywhere in the sideways region &8000-&BFFF is latched into that
        // RAM directly, regardless of which ROM is currently paged in (and without
        // disturbing it). write_bank() honours the slot's write-protect switch, so
        // a protected write is silently swallowed here rather than falling through
        // to the paged bank. Reads are left to the normal ROMSEL-gated path, so the
        // RAM only reads back when slot 15 is selected via &FE30.
        //
        // When slot 15 holds a ROM (or nothing), there is no write-through and the
        // write follows the normal path, where the ROM/empty socket ignores it.
        if (addr >= 0x8000 && addr <= 0xBFFF &&
            sideways.bank_type(SLOT_15) == SlotType::Ram) {
            sideways.write_bank(SLOT_15, static_cast<uint16_t>(addr - 0x8000), value);
            return;
        }
    ...
    // Per-bank write-protect control, surfaced to SidewaysService. On this board
    // only slot 15 can hold RAM, so only slot 15 is meaningfully protectable; the
    // service rejects requests for non-RAM slots.
    ...
    // Topology of the 16 sideways slots on a Model B fitted with the Sidewise.
    //
    // Slots 0-3 are the relocated motherboard sockets and slots 4-14 are the
    // board's ROM sockets: all ROM-only. Slot 15 is the single RAM/ROM socket.
    // No slot is runtime-configurable: which chip is in which socket, and whether
    // slot 15 is fitted with RAM or a ROM, is set at launch (like fitting a chip)
    // - the only runtime control on the real board is the slot-15 write-protect
    // switch (see set_slot_write_protected).
    static SlotTopology slot_topology(MotherboardLinks /*links*/ = {}) {
        ...
        for (int slot = 0; slot < 16; ++slot) {
            ...
            if (slot == SLOT_15) {
                ...
                spec.label = "Sidewise ROM " + std::to_string(slot);
                spec.supports_ram = false;
            }
