...
// BBC Model B fitted with the Watford Electronics ROM/RAM board.
//
// This emulates a specific historical product (Watford Electronics, 1985) rather
// than a notional board. It expands the four stock sideways sockets to the full
// sixteen supported by the OS, with a distinctive mix of RAM and ROM sockets:
// - Slots 0-7 are the board's dynamic sideways RAM (DRAM), fitted 32K/64K/128K
//   (2/4/8 banks) at launch. RAM or empty.
// - Slots 8-11 are the four original motherboard sockets, ROM only, lower
//   priority than the board's own sockets.
// - Slots 12, 13, 15 are the board's ROM sockets.
// - Slot 14 is the board's ROM/static-RAM socket (two 8K devices; on the real
//   board optionally battery-backed). We model it as one 16K slot fitted with
//   RAM or a ROM at launch. Battery backup is not modelled - RAM is volatile.
//
// Three behaviours distinguish this from a plain banked-memory board:
//
//  1. A separate write-select latch at &FF30. In addition to the normal ROMSEL
//     read-select latch at &FE30, a write to &FF30 + N latches N (the low nibble
//     of the ADDRESS; the data written is irrelevant) as the socket that all
//     subsequent writes to &8000-&BFFF are directed to - independently of which
//     bank is paged in for reading. Reads of &FF3X are unaffected (MOS ROM). This
//     lets software *LOAD or assemble straight into a chosen RAM bank, and gives
//     a temporary write-protect for free: selecting a ROM socket (e.g. ?&FF38=0)
//     makes writes go nowhere.
//
//  2. S2 - a global write-protect link, commonly wired to a switch. When engaged
//     it inhibits all writes to the sideways region regardless of the write
//     latch (a permanent counterpart to the ?&FF38 trick).
//
//  3. S1 - a read-protect link for socket 14. When engaged (link removed) socket
//     14 "vanishes", reading as a constant, a recovery route for battery-backed
//     RAM holding software that hangs the machine.
//
// Consistent with modelling a real board, slot population is fixed at launch (via
// CLI / preset, like fitting a chip). The runtime controls are the write-select
// latch (driven by the guest) and the S1/S2 switches.
//
class ModelBWatfordRomRamHardware {
public:
    // Slot 14 is the board's ROM/static-RAM socket; S1 read-protects it.
    static constexpr uint8_t SLOT_14 = 14;

    // Machine identification and region names (compile-time constants)
    static constexpr std::string_view MACHINE_TYPE = "model-b-watford-rom-ram";
    constexpr std::string_view machine_type() const { return MACHINE_TYPE; }
    static constexpr std::string_view MACHINE_DISPLAY_NAME = "BBC Model B with Watford ROM/RAM board";
    static constexpr std::string_view MACHINE_DESCRIPTION = "Model B fitted with the Watford Electronics ROM/RAM board (8 sideways RAM banks 0-7, ROM sockets 8-15, a &FF30 write-select latch, and S1/S2 links)";
    ...
    // The Watford manual recommends fitting BASIC in socket 15 (the middle of the
    // board's five large sockets), the highest-priority ROM socket.
    static constexpr uint8_t DEFAULT_LANGUAGE_SLOT = 15;
    // No DEFAULT_DFS_ROM: the board is empty expansion sockets with no filing
    // system of its own. A DFS (or the Watford SFS) is provisioned by a preset
    // via its sideways_bank section, not auto-loaded by the machine.
    ...
    // The &FF30 write-select latch: the socket that writes to &8000-&BFFF are
    // directed to, independent of the &FE30 read-select. The 74LS75 latch has no
    // reset line on the board (power-on state is indeterminate); we default it to
    // 0 deterministically. Software always sets it before relying on it.
    uint8_t write_select_ = 0;

    // S2 - the global write-protect link/switch. When engaged, all writes to the
    // sideways region are inhibited regardless of the write-select latch.
    bool write_protect_switch_ = false;

    // S1 - the socket-14 read-protect link/switch. When engaged (link removed),
    // socket 14 "vanishes": reads of it yield a constant (open bus).
    bool bank14_read_protect_ = false;

    // Value a "vanished" (S1-engaged) socket 14 reads back as.
    static constexpr uint8_t READ_PROTECT_VALUE = 0xFF;
    ...
    ModelBWatfordRomRamHardware(ViaPeripheral& system_peripheral, ViaPeripheral& user_peripheral)
    ...
    uint8_t read(uint16_t addr) {
        // S1 read-protect: when engaged, socket 14 "vanishes" - reads of the
        // sideways region while socket 14 is paged in yield a constant. Reads of
        // every other bank, and of the rest of the address space, are unaffected.
        if (bank14_read_protect_ && addr >= 0x8000 && addr <= 0xBFFF &&
            sideways.selected_bank() == SLOT_14) {
            return READ_PROTECT_VALUE;
        }
        return memory_map_.read(addr);
    }

    void write(uint16_t addr, uint8_t value) {
        // The &FF30 write-select latch. A write to &FF30-&FF3F strobes the latch,
        // latching the low nibble of the ADDRESS as the write-target socket; the
        // data byte is irrelevant. Reads of &FF3X are left to the normal path
        // (MOS ROM), so this intercepts writes only.
        if (addr >= 0xFF30 && addr <= 0xFF3F) {
            write_select_ = static_cast<uint8_t>(addr & 0x0F);
            return;
        }
        // Writes to the sideways region go to the write-SELECTED socket, not the
        // ROMSEL-paged (read) bank - this is the board's defining behaviour. The
        // S2 global write-protect switch inhibits them entirely. write_bank()
        // honours the target slot's type, so a write to a ROM/empty socket (e.g.
        // after ?&FF38=0) is silently ignored - the temporary write-protect trick.
        if (addr >= 0x8000 && addr <= 0xBFFF) {
            if (!write_protect_switch_) {
                sideways.write_bank(write_select_, static_cast<uint16_t>(addr - 0x8000), value);
            }
            return;
        }
    ...
    // The current write-select latch value (the &FF30-selected socket).
    uint8_t write_select_socket() const { return write_select_; }

    // S2 - global write-protect switch. Settable at launch and toggleable at
    // runtime (a switch is commonly wired to the S2 link); surfaced on
    // SidewaysService.
    bool global_write_protect() const { return write_protect_switch_; }
    void set_global_write_protect(bool engaged) { write_protect_switch_ = engaged; }

    // S1 - socket-14 read-protect switch.
    ...
    void reset() {
        ...
        // The read-select is zeroed on a hard reset; mirror that for the
        // write-select latch so the machine starts from a deterministic state.
        // The S1/S2 switches persist - they are physical board switches, not
        // reset by Break. (soft_reset leaves both selects untouched, as ROMSEL is.)
        write_select_ = 0;
        disc_socket.reset();
        econet_socket.reset();
        serial_socket.reset();
    ...
    // Topology of the 16 sideways slots on a Model B fitted with the Watford
    // ROM/RAM board:
    // - slots 0-7  : the board's dynamic sideways RAM (RAM or empty),
    // - slots 8-11 : the four original motherboard sockets (ROM or empty),
    // - slots 12,13,15 : the board's ROM sockets (ROM or empty),
    // - slot 14    : the board's ROM/static-RAM socket (ROM, RAM or empty).
    // No slot is runtime-configurable: which chip is in which socket is set at
    // launch (like fitting a chip). The runtime controls are the &FF30 write
    // latch (driven by the guest) and the S1/S2 switches (board-level, not
    // per-socket - see set_global_write_protect / set_bank14_read_protect).
    static SlotTopology slot_topology(MotherboardLinks /*links*/ = {}) {
        ...
        for (int slot = 0; slot < 16; ++slot) {
            ...
            if (slot <= 7) {
                ...
                spec.supports_rom = false;  // DRAM banks, not ROM sockets
            } else if (slot <= 11) {
                spec.label = "Motherboard ROM " + std::to_string(slot);
                spec.supports_ram = false;
                spec.supports_rom = true;
            } else if (slot == SLOT_14) {
                spec.label = "Board ROM/RAM " + std::to_string(slot);
                spec.supports_ram = true;   // two 8K static-RAM/ROM sockets
                spec.supports_rom = true;
            } else {  // slots 12, 13, 15
                spec.label = "Board ROM " + std::to_string(slot);
                spec.supports_ram = false;
                spec.supports_rom = true;
            }
