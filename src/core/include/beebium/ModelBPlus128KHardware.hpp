...
// BBC Model B+ 128K hardware configuration.
//
// Identical to the Model B+ 64K (see ModelBPlusHardware.hpp) except that
// the four sideways pages opposite IC71 + at slots 12/13 are sideways
// RAM, totalling an additional 64 KiB. See Acorn Support Group
// Application Note 030 ("BBC B+ 128K - Additional User Information") for
// the user-facing description; see the B+ Service Manual sec. 5.4 for
// the hardware.
//
// RAM bank positions, per AN 030:
//   - Always at slots 12 and 13 (DFS calls these W and X)
//   - At slots 0 and 1 when S13=South (DFS calls these Y and Z) - BASIC
//     is at slots 14 and 15
//   - At slots 14 and 15 when S13=North (DFS Y and Z) - BASIC at 0 and 1
//
// W/X/Y/Z are software aliases supplied by DFS 2.26's SRAM utilities;
// the emulator just exposes four RAM banks at the correct slot numbers
// and DFS handles the alphabetic translation.
//
// THE REST OF THIS FILE MIRRORS ModelBPlusHardware.hpp. Differences
// versus the 64K are localized to:
//   - The four w_ram/x_ram/y_ram/z_ram members
//   - The SidewaysType template pack (4 extra Ram bindings)
//   - The sideways{} construction (4 extra make_bank calls)
//   - slot_topology() emits four extra SocketSpec entries
//   - apply_motherboard_links() rebinds the Y/Z RAM pair based on S13
// Keep both files in sync for everything else.
//
...
public:
    ...
    // the slot pair opposite BASIC (so {0, 1} when S13=South, {14, 15}
    // when S13=North). The Y/Z banks are wired into their final slot
    // numbers by apply_motherboard_links() at startup.
    Ram<16384> sram_w;             // SRAM W - slot 12 (always)
    Ram<16384> sram_x;             // SRAM X - slot 13 (always)
    ...
    // Construction-time bindings reflect the factory-default S13=South
    // configuration: BASIC at slots 14/15, SRAM W/X always at 12/13,
    // SRAM Y/Z at slots 0/1. apply_motherboard_links() may rebind the
    // Y/Z pair to {14, 15} and BASIC to {0, 1} at startup when S13=North.
    ...
    SidewaysType sideways{
        make_bank<15>(basic_rom),    // IC71 - slot 15 (BASIC, S13=South default)
...
public:
    ...
    ModelBPlus128KHardware(ViaPeripheral& system_peripheral, ViaPeripheral& user_peripheral)
...
public:
    ...
    //
    // On the B+ 128K the four sideways RAM banks fill slots 12 and 13
    // (always) plus the slot pair opposite IC71 (so {0, 1} with
    // S13=South or {14, 15} with S13=North); see AN 030. These are
    // exposed as the SRAM W/X/Y/Z "sockets" in the topology even
    // though they're soldered down rather than physical IC sockets,
    // because the gRPC view of "slots that hold RAM/ROM" is the same
    // shape regardless of whether the RAM is socketed.
    //
    // The six ROM sockets (IC35..IC71) are ROM-only and not
    // runtime-configurable. The four RAM banks are RAM-only and not
    // runtime-configurable (the contents are mutable via CPU writes
    // and *SRLOAD, but the type is fixed by the hardware).
    ...
    static SlotTopology slot_topology(MotherboardLinks links) {
        SlotTopology topo;
        topo.has_aliasing = true;  // IC sockets respond to slot pairs

        // BASIC half of IC71 + the movable Y/Z RAM pair both follow S13:
        // they sit in opposite pairs.
        std::vector<int> ic71_slots;
        std::vector<int> sram_y_slots;  // single slot - non-aliased RAM bank
        std::vector<int> sram_z_slots;
        switch (links.s13) {
        ...
        const std::vector<Entry> entries = {
            ...
            {"SRAM Z", sram_z_slots, true}      // RAM opposite IC71
        };
    ...
    void apply_motherboard_links(const MotherboardLinks& links) {
        ...
        switch (links.s13) {
            case MotherboardLinks::S13Position::South:
                // Already in this configuration from construction. No-op,
                // explicit case so the switch is exhaustive and the intent
                // is documented.
                break;
            case MotherboardLinks::S13Position::North:
                sideways.rebind_bank(0, basic_rom);
                sideways.rebind_bank(1, basic_rom);
                sideways.rebind_bank(14, sram_y);
                sideways.rebind_bank(15, sram_z);
                break;
        }
    }
