...
`oaknut-disc`). The build is **byte-reproducible**: it pins all datestamps to a
fixed era constant, so re-running the script yields a `.tar.xz` byte-identical
to the committed one whenever the parts and the pinned `oaknut` version are
unchanged. That is the maintenance check -- rebuild and `cmp` against the
committed artifact.
